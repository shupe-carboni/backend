...
err_desc_template = (
    "{link_type} link {primary}/{id_}/{secondary} has not been implemented"
)
responses = {
...
def get_related_handler(
    ...
    msg = err_desc_template.format(
        link_type=link_type, primary=primary, id_=id_, secondary=secondary
    )
    raise HTTPException(status_code=status.HTTP_501_NOT_IMPLEMENTED, detail=msg)


@relationships.get(
    "/{primary}/{id_}/relationships/{secondary}",
    tags=["Not Implemented"],
    status_code=status.HTTP_501_NOT_IMPLEMENTED,
)
